...
console.log(
  `Formula: ${annualVolume.toLocaleString()} ÷ ${dailyCapacity.toFixed(2)}`
);
...
console.log(
  `- The machine can produce ${dailyCapacity.toFixed(0)} pieces per day`
);
console.log(
  `- To produce ${annualVolume.toLocaleString()} pieces annually, you need ${utilizationDays.toFixed(
    1
  )} days`
);
console.log(
  `- This means the machine will be utilized for ${(
    (utilizationDays / 365) *
    100
  ).toFixed(1)}% of the year`
);
...
examples.forEach((example, index) => {
  const capacity = calculateDailyProductionCapacity(
    example.cavities,
    example.cycleTime,
    example.oee
  );
  ...
  console.log(
    `  ${example.cavities} cavities, ${example.cycleTime}s cycle, ${(
      example.oee * 100
    ).toFixed(0)}% OEE`
  );
  console.log(`  Daily Capacity: ${capacity.toFixed(0)} pieces/day`);
  console.log(
    `  Utilization: ${days.toFixed(1)} days (${((days / 365) * 100).toFixed(
      1
    )}% of year)\n`
  );
});
