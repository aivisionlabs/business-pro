...
describe('FreeCashFlow', () => {
  ...
  it('displays correct FCF formula', () => {
    ...
    expect(screen.getByText(/FCF = EBIT × \(1 - Tax Rate\) \+ Depreciation - Δ Working Capital/)).toBeInTheDocument();
  ...
  it('shows working capital impact explanation', () => {
    ...
    expect(screen.getByText(/Working capital changes represent the cash tied up in operations/)).toBeInTheDocument();
  });
});
